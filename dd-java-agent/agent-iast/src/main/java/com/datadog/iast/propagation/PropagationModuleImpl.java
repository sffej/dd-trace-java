...
public class PropagationModuleImpl implements PropagationModule {
  ...
  public void taintObjectRange(
    ...
    internalTaint(to, target, new Range[] {range}, NOT_MARKED, true);
  }
  ...
  public Taintable.Source findSource(
      @Nonnull final IastContext ctx, @Nullable final Object target) {
    if (target == null) {
      return null;
    }
    final TaintedObjects to = ctx.getTaintedObjects();
  ...
  @Override
  public boolean isTainted(@Nonnull final IastContext ctx, @Nullable final Object target) {
  ...
  private static void internalTaint(
      ...
      final int mark) {
    internalTaint(to, value, ranges, mark, false);
  }
