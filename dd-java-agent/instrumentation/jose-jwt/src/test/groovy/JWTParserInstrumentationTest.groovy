...
class JWTParserInstrumentationTest extends AgentTestRunner {
  ...
  void 'oauth jwt parser'() {
    ...
    then:
    1 * propagationModule.taintObject(iastCtx, payload, SourceTypes.REQUEST_HEADER_VALUE)
    0 * _
  }
  ...
  void 'jose JSONObjectUtils instrumentation'() {
    ...
    then:
    1 * propagationModule.taintObject(iastCtx, 'http://foobar.com', SourceTypes.REQUEST_HEADER_VALUE, 'iss')
