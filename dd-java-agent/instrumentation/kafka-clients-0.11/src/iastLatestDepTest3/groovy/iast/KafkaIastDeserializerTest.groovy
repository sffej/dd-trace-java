...
class KafkaIastDeserializerTest extends AgentTestRunner {
  ...
  void 'test string deserializer: #test'() {
    ...
    switch (test.method) {
      ...
      case Method.WITH_BYTE_BUFFER:
        1 * propagationModule.taintObject(_ as ByteBuffer, source, 0, payload.length) // taint ByteBuffer
        1 * propagationModule.taintStringIfTainted(payload, _ as ByteBuffer, 0, payload.length, false, NOT_MARKED) // taint ByteBuffer => byte[]
        1 * codecModule.onStringFromBytes(payload, 0, payload.length, _, _ as String) // taint byte[] => string
        break
      case Method.WITH_BYTE_BUFFER_OFFSET:
        1 * propagationModule.taintObject(_ as ByteBuffer, source, BUFF_OFFSET, payload.length) // taint ByteBuffer
  ...
  void 'test byte array deserializer: #test'() {
    ...
    switch (test.method) {
      ...
      case Method.WITH_BYTE_BUFFER_OFFSET:
        1 * propagationModule.taintObject(_ as ByteBuffer, source, BUFF_OFFSET, payload.length) // taint ByteBuffer
        1 * propagationModule.taintStringIfTainted(payload, _ as ByteBuffer, BUFF_OFFSET, payload.length, false, NOT_MARKED) // taint ByteBuffer => byte[]
        break
    }
    0 * _
  ...
  void 'test json deserialization: #test'() {
    ...
    switch (test.method) {
      ...
      case Method.WITH_BYTE_BUFFER:
        1 * propagationModule.taintObject(_ as ByteBuffer, source, 0, payload.length) // taint ByteBuffer
        1 * propagationModule.taintStringIfTainted(payload, _ as ByteBuffer, 0, payload.length, false, NOT_MARKED) // taint byte[] => ByteBuffer
        break
      case Method.WITH_BYTE_BUFFER_OFFSET:
        1 * propagationModule.taintObject(_ as ByteBuffer, source, BUFF_OFFSET, payload.length) // taint ByteBuffer
        1 * propagationModule.taintStringIfTainted(payload, _ as ByteBuffer, BUFF_OFFSET, payload.length, false, NOT_MARKED) // taint byte[] => ByteBuffer
        break
    }
    // taint JSON
    ...
    1 * propagationModule.taintObject(_, 'name', source, 'name', json)
