...
class CaseInsensitiveHeadersInstrumentationTest extends AgentTestRunner {
  ...
  void 'test that get() is instrumented'() {
    ...
    then:
    1 * module.taintObjectIfTainted(iastCtx, 'value', headers, SourceTypes.REQUEST_PARAMETER_VALUE, 'key')
  }
  ...
  void 'test that entries() is instrumented'() {
    ...
    result.collect { it.key }.unique().each {
      1 * module.taintObject(iastCtx, it, SourceTypes.REQUEST_PARAMETER_NAME, it)
    }
    result.each {
      1 * module.taintObject(iastCtx, it.value, SourceTypes.REQUEST_PARAMETER_VALUE, it.key)
    }
  }
