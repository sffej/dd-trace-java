...
class MultipartInstrumentationForkedTest extends AgentTestRunner {
  ...
  void 'test getName'() {
    ...
    then:
    1 * module.taintObject(iastCtx, 'partName', SourceTypes.REQUEST_MULTIPART_PARAMETER, 'Content-Disposition')
    0 * _
  }
  ...
  void 'test getHeader'() {
    ...
    then:
    1 * module.taintObject(iastCtx, 'headerValue', SourceTypes.REQUEST_MULTIPART_PARAMETER, 'headerName')
    0 * _
  }
  ...
  void 'test getHeaderNames'() {
    ...
    then:
    1 * module.taintObject(iastCtx, 'headerName', SourceTypes.REQUEST_MULTIPART_PARAMETER)
    0 * _
  }
